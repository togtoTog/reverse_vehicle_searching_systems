...
def load_dataset():
    ...
    train_dataset = ImageFolder(type_cfg.TRAIN.DATASET,
                                transform=train_transform)  # 训练集数据
    train_loader = torch.utils.data.DataLoader(dataset=train_dataset, batch_size=type_cfg.TRAIN.BATCH_SIZE, shuffle=True,
                                               num_workers=2)  # 加载数据
    ...
    test_dataset = ImageFolder(type_cfg.TEST.DATASET,
                               transform=test_transform)  # 测试集数据
    test_loader = torch.utils.data.DataLoader(dataset=test_dataset, batch_size=type_cfg.TRAIN.BATCH_SIZE, shuffle=False,
                                              num_workers=2)  # 加载数据
...
def train_init():
    # 设置模型结构
    model = models.resnet50(pretrained=False).to(type_cfg.MODEL.DEVICE)
    # 加载预训练模型参数
    model.load_state_dict(torch.load(type_cfg.MODEL.PRETRAIN))
...
def train_and_valid(train_loader, test_loader, train_data_size, valid_data_size, model, loss_function, optimizer,
    ...
    device = torch.device(type_cfg.MODEL.DEVICE)
    history = []
    best_acc = 0.0
    best_epoch = 0
    ...
    for epoch in range(epochs):
        ...
        torch.save(model.state_dict(), os.path.join(
            type_cfg.MODEL.CHECKPOINT, f'model_{epoch + 1}.pth'))
...
if __name__ == '__main__':
    ...
    history = train_and_valid(train_loader, test_loader, train_data_size, valid_data_size,
                    model, loss_function, optimizer, type_cfg.TRAIN.EPOCHS)
    # 将训练参数用图表示出来
    ...
    plt.ylabel('Loss')
    plt.ylim(0, 1.1)
    plt.savefig(type_cfg.METRIC.LOSS_PIC)
    # plt.show()
    ...
    plt.ylabel('Accuracy')
    plt.ylim(0, 1.1)
    plt.savefig(type_cfg.METRIC.ACCURACY_PIC)
